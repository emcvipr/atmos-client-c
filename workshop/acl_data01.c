...
void getumd_action_cmdline(credentials *creds, char *object_id, ws_result *wsr);
...
void create_usage() 
{
    fprintf(stderr, "Usage:\n"
	    ...
	    "--usracl user=perm FULL_CONTROL|READ|WRITE\n"
	    "--groupacl \n"
	    ""	    
	    );
}
...
int main(int argc, char *argv[])
{
	...
	c = getopt_long(argc, argv, "t:s:h:H:o:rcui:m:gA:G:",long_options, &option_index);
	if (c == -1)
	    break;
	switch (c) {
	case 0:
	...
	case 'A':
	    ...
	    int acl_pos = 0;
	    int acl_found = 0;
	    for(acl_pos; acl_pos < strlen(optarg); acl_pos++) {
		if(optarg[acl_pos] == '=') {
		    if(!current_acl)  {
			acl = malloc(sizeof(acl));
			current_acl = acl;
		    }  else {
			while(current_acl->next)  {
			    current_acl=current_acl->next;
			}
			current_acl->next=malloc(sizeof(acl));
			current_acl=current_acl->next;
		    }

		    strncpy(acl->user, optarg, acl_pos);
		    strncpy(acl->permission, optarg+acl_pos+1, strlen(optarg)-acl_pos);
		    acl_found = 1;
		}
	    }
	    if(acl_found != 1) {
		printf("malformed permissions\n");
		exit(EXIT_FAILURE);
	    }
	    free(acl);
	    break;
	case 'G':
	    ...
	    int gpos = 0;
	    int gfound = 0;
	    for(gpos; gpos < strlen(optarg); gpos++) {
		if(optarg[gpos] == '=') {
		    char group[1024];
		    char perm[1024];
		    memset(group, 0, 1024);
		    memset(perm, 0, 1024);		
		    strncpy(group, optarg, gpos);
		    strncpy(perm, optarg+gpos+1, strlen(optarg)-gpos);		    
		    gfound = 1;
		}
	    }
	    if(gfound != 1) {
...
void do_atmos_action(enum atmos_cmd cmd, credentials *creds,char *object_id, char* bytes, acl *acl, user_meta* umd, int limit, ws_result *wsr) 
{
	...
	break;
    case USERMETA:
	getumd_action_cmdline(creds, object_id, wsr);
	break;
    default:
